...
def exp_on_n_and_h():
    ...
    for h in hs:
        for n in ns:
            ...
            ax[0].scatter(np.log(h), ori_pi, color="blue")
            ax[0].scatter(np.log(h), improved_pi, color="red")
            ax[1].scatter(np.log(n), ori_pi, color="blue")
            ax[1].scatter(np.log(n), improved_pi, color="red")
    ax[0].set_xlabel("h (log scale)")
    ax[0].set_ylabel("pi")
    ...
    ax[0].hlines(y=np.pi, xmin=0, xmax=10, color="black", linestyle="--")
    ax[1].hlines(y=np.pi, xmin=0, xmax=10, color="black", linestyle="--")
    plt.savefig("p3.png")
