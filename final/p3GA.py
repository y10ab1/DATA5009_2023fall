...
def schedule(chromosome, idx, gantt, job_status_machine, job_status_time):
    ...
    for machine in range(5):
        try:
            ...
            logger.error(f"Chromosome: {chromosome}")
            logger.error(f"Invalid task or transfer table data: O{jobid}{step}, M{machine+1}")
            logger.error(f"job_status_machine: {job_status_machine[jobid-1]}, machine: {machine+1}")
            return -1
    ...
    return update_gantt(jobid, step, selected_machine, min_time_cost, gantt, job_status_machine, job_status_time)
...
def update_gantt(jobid, step, machine, time, gantt, job_status_machine, job_status_time):
    ...
    if current_time + time <= gantt.shape[1]:
        ...
        logger.warning(f"Unable to schedule job {jobid} at step {step} due to time constraints: {current_time + time}")
        return False
...
def schedule_chromosome(chromosome, gantt, job_status_machine, job_status_time):
    ...
    for idx in range(len(chromosome)):
        if not schedule(chromosome, idx, gantt, job_status_machine, job_status_time):
            return []
    return gantt
...
def fill_remaining(child, parent, start, end):
    size = len(child)
    toremove, cnt = np.unique(child[start:end], return_counts=True)
    # remove same number of elements that already exist in the selected subset
    tofill = parent.copy()
    for number, c in zip(toremove, cnt):
        tofill = np.delete(tofill, np.where(tofill == number)[0][:c])
    # fill the remaining elements
    tofill = tofill.tolist()
    for i in range(size):
        if child[i] == -1:
...
# Main function
def GA(args, pop_size=10, max_generations=1, mutation_rate=0.1):
